function AssetViewModel(props) {
  ...
  self.balance = ko.observable(props['initialBalance'] || 0); //raw
  ...
  self.displayedBalance = ko.computed(function() {
    return numberWithCommas(self.DIVISIBLE ? toFixed(self.balance() / UNIT, 8) : self.balance()).toString(); 
  }, self);
  
  self.displayedTotalIssued = ko.computed(function() {
    return numberWithCommas(self.DIVISIBLE ? toFixed(self.totalIssued() / UNIT, 8) : self.totalIssued()).toString(); 
  }, self);

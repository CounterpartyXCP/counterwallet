...
function AddressViewModel(type, key, address, initialLabel, pubKeys) {
  ...
  self.assets = ko.observableArray([
    ...
    new AssetViewModel({address: address, asset: "XCP"}),
    new AssetViewModel({address: address, asset: "BITCRYSTALS"}), //will be updated with data loaded from counterpartyd
  ]);
  ...
  self.SOGfilteredAssets = ko.computed(function(){
    if(self.assetFilter() == '') { //show all
      return self.assets();
    } else if(self.assetFilter() == 'base') {
      return ko.utils.arrayFilter(self.assets(), function(asset) {
